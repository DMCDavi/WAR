...
void Funcao_ataque_defesa (int qtd_dados_ataque, int qtd_dados_defesa){
	...
	while (pais_desejado_atacante != '0' || pais_desejado_atacado != '0'){ // LOOPING DO ATAQUE
		...
		if (teste_dono_pais (pais_desejado_atacante)){ //caso o pais pertenca ao player q ataca...
			...
			//pesquisa do paÌs correspondente 
			...
			//far· a verificacao se o numero de tropas eh suficiente para atacar
			if (tropas_pais[posicao_pais_encontrado_ataque] != 1) { //caso a qtd de tropas seja suficiente...
				...
				if (teste_dono_pais (pais_desejado_atacado)) { //se pertencer ao inimigo
					//caso o pais √É¬± pertenca ao player q defende...
					...
					// colocar funcao de teste de vizinhan√ßa
					...
					if(maior_ataque[2]<= maior_defesa[2]){ //perda ataque
						tropas_pais[posicao_pais_encontrado_ataque] = tropas_pais[posicao_pais_encontrado_ataque] - 1; //reduÁ„o de tropas
					}
					
					else{ //ganho ataque
						tropas_pais[posicao_pais_encontrado_defesa] = tropas_pais[posicao_pais_encontrado_defesa] - 1; //reduÁ„o de tropas
						if(tropas_pais[posicao_pais_encontrado_defesa] == 0){ //se o a defesa Ò tiver mais tropas...
					...
					// o "!= 0" È para que Ò compare com as posicoes do vetor q Ò foram preenchidas, caso a defesa ou ataque Ò possua 3 dados
						...
						if(tropas_pais[posicao_pais_encontrado_defesa] == 0){ //se o a defesa Ò tiver mais tropas...
							if(strcmp(player[vez], player1) == 0){ //se for a vez do player 1
								pais[posicao_pais_encontrado_defesa][0] = '1'; //player1 conquista o territ√≥rio do player2
							}
							else if(strcmp(player[vez], player2) == 0){ //se for player 2 
								pais[posicao_pais_encontrado_defesa][0] = '2'; //player2 conquista o territorio do player1
							}
				...
				else{ //caso o paÌs Ò pertenÁa ao inimigo
		...
		else{ //caso o pais √± perten√ßa ao jogador q est√° jogando
			printf("[ERRO] Nao eh possivel selecionar esse pais! Por favor, tente outro.\n");
		}
