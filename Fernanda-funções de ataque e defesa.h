...
void ataque (int qtd_dados_ataque, qtd_dados_defesa){
	...
	while (pais_desejado_atacante != '0' || pais_desejado_atacado != '0'){ 
		...
		if teste_dono_pais (pais_desejado_atacante){ //se pertencer
			if (tropas_pais[num_pais] != 1) { //relacionar tropas_pais com a variavel posicao_pais
				//em ferramentas fazer uma funcao para verificacao de qtd de tropas
		...
		if(qtd_dados_defesa == 2 && qtd_dados_ataque == 1){
			...
			else printf("Nao eh possivel selecionar esse pais. Por favor, tente outro.\n");
				}
			
			else { //caso o pais ñ pertenca ao player q ataca...
